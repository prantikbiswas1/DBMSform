<?php
    ...
    if (isset($_POST["submit"])) { 
        ...
        select * from `page4` where `email` = '$email'
        ");
        ...
        if($data){
            ...
            delete from `page4` where `email`='$email'
            ");
            $deleteUser->execute();
        }

        $insertjson = $conn->prepare("insert into `page4` (`email`,`jsondata`) values ('$email','$jsondata')");
        $insertjson->execute();
        
        if ($insertjson) {
            echo "Inserted successfully";
            header("Location: /DBMSform/fifthpage.php");
        }  
    }
...
<div class="form-group">
  ...
  <div class="col-md-1">
    <a href="./thirdpage.php" class="btn btn-primary pull-left">Back</a>
  </div>
